...
export default function ChatInputBar({ onSend }: { onSend: (text: string, attachments: ChatAttachment[]) => void }) {
  ...
  return (
    <form className="sticky bottom-24 z-30 space-y-3 rounded-2xl border border-slate-700 bg-slate-950/95 p-2 shadow-[0_-18px_48px_rgba(2,6,23,0.55)] backdrop-blur lg:bottom-0 lg:border-t lg:shadow-none" onSubmit={submit}>
      {attachments.length ? (
      ...
      <div className="grid gap-2 rounded-2xl border border-slate-700 bg-slate-900/90 p-2 sm:flex sm:items-end">
        ...
        <textarea className="max-h-36 min-h-14 w-full resize-none bg-transparent px-2 py-3 text-base font-semibold text-white outline-none placeholder:text-slate-600 sm:flex-1 sm:text-sm" value={text} onChange={(event) => setText(event.target.value)} onPaste={handlePaste} placeholder="พิมพ์แผนเทรด หรือ paste รูปกราฟ..." rows={1} />
