...
export default function CapitalLedger({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section className="trade-card min-w-0 rounded-3xl p-4">
      <div className="min-w-0">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-trade-green">Capital Ledger</p>
      ...
      {!compact ? (
        <div>
          ...
          <form className="mt-4 grid min-w-0 gap-2 sm:grid-cols-2" onSubmit={submit}>
            <select className="min-h-12 min-w-0 rounded-xl border border-white/10 bg-[#111314] px-3 font-bold text-white outline-none focus:border-trade-green/60" value={type} onChange={(event) => setType(event.target.value as CapitalTransactionType)}>
              <option value="deposit">ฝากทุน</option>
              <option value="withdraw">ถอนทุน</option>
            </select>
            <input className="min-h-12 min-w-0 rounded-xl border border-white/10 bg-[#111314] px-3 font-bold text-white outline-none placeholder:text-slate-500 focus:border-trade-green/60" inputMode="decimal" placeholder="จำนวน USD" type="number" value={amountUsd} onChange={(event) => setAmountUsd(event.target.value)} />
            <input className="min-h-12 min-w-0 rounded-xl border border-white/10 bg-[#111314] px-3 font-bold text-white outline-none placeholder:text-slate-500 focus:border-trade-green/60 sm:col-span-2" placeholder="โน้ต เช่น เติมทุน" value={note} onChange={(event) => setNote(event.target.value)} />
            <button className="trade-button min-h-12 rounded-xl px-4 font-black sm:col-span-2" type="submit">บันทึก</button>
          </form>
          <div className="mt-3 grid gap-2">
            {items.slice(0, 5).map((item) => (
              <div className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/20 p-3 text-sm" key={item.id}>
                <div className="min-w-0">
...
function SummaryLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/20 px-3 py-2">
      <p className="shrink-0 text-xs font-bold text-slate-400">{label}</p>
